...
def buy():
    if request.method == "GET":
        ...
        if not request.form.get("shares"):
            return apology("Invalid Number of Stocks", 403)
        try:
            shares = int(request.form.get("shares"))
        except ValueError:
            return apology("Invalid Number of Stocks", 403)
        if not request.form.get("symbol"):
            return apology("no symbol found", 403)
        stock = lookup(symbol)
        if not stock:
...
def changepass():
    if request.method == "POST":
        current = request.form.get("oldpass")
        new = request.form.get("newpass")
        comfirm = request.form.get("compass")
        ...
        new_pass=generate_password_hash(new)
        db.execute("UPDATE user SET hash = ? where id = ?", new_pass, session["user_id"])
        return redirect("/")
